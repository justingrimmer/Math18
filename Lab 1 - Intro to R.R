################  LAB 1: INTRO TO R  ################
### Stanford Poli Sci. Math Camp, 2014
## Jonathan Mummolo and Erik Peterson, TAs
...
paste("Welcome to Stanford! You are one in a ", sample(c("million!","billion!","trillion!","cohort of roughly 20? (Idk. Not great with numbers.)"), size=1), sep="")


############Notice the features of the output R produces. First we see "[1]". This indicates that whatever follows is the first element of the output. Sometimes output will have multiple elements, but here there is just one. Then we see the contents of that object: a sentence generated by the code we executed. On the line below that, we see a ">" symbol. If you're like us, and don't get out much, you will come to know this symbol as a dear friend. It indicates that the code was executed smoothly, and R is ready for your next command. If something had gone wrong, we would most likely have gotten an error message between the output and the ">", or not seen the ouput at all.
...
setwd("/Users/jonathanmummolo/Dropbox/Math Camp 2014/Math Camp Labs (FINAL)/Data")##insert your file path between the quote marks
...
############ R is the last calculator you will ever need. It can perform virtually any style of computation, including calculus and matrix algebra operations. There is not time for a full treatment of all the types of commands here, but just to get things going, we'll start with a few simple examples.
...
###Note: to display the contents of objects inside for loops, which we will cover later, we need to use the "print()" function.
...
##Note that in practice, you would do this in one line of code by "nesting" the functions. That's right, functions inside functions. (Mind: Blown. (WHAA?))
...
##We use the "as.numeric()" and "as.character()" functions a ton, but they are kind of cumbersome to write. So I often rename these functions as follows:

as.c<-as.character
as.n<-as.numeric

nums.c<-as.c(nums) ##let's check to make sure the function still works
class(nums.c)
nums.c
...
##Beginning to see the power of this organizational system? If not, it will soon become apparent. We can even have lists within lists within lists, each containing myriad objects, and if we know how to index properly, we can access any element of these complex data structures in an instant. No need to keep multiple files open or switch screens. It's all at our fingertips.  
...
################ HOW TO USE R TO SCRAPE DATA OFF THE INTERNET
###### WARNING: SOME WEB SITES HAVE RULES ABOUT SCRAPING DATA. ALWAYS READ TERMS OF SERVICE/CONSULT WITH A PROFESSOR BEFORE USING THIS TECHNIQUE TO AVOID GETTING SUED, MAIMED, ETC.


###### THE DATA WE GRAB BELOW IS NOT PROPRIETARY, SO WE ARE FINE


##### THE DEPT. OF LABOR PUBLISHES PRELIMINARY UNEMPLOYMENT DATA IN PRESS RELEASES. These differ from the unemployment data that gets archived later (which are revised estimates) but for this project we need the numbers as originally announced. 

####The rates are displayed in tables on this web site, but the tables are coded in html, not in downloadable data tables. We wanted monthy unemployment rates as originally released by the government, and this is the only way they are available online. We could hand copy each unemployment rate, but that would be a huge time sink, and probably prone to human error. We can have R grab the data for us.


##We'll need the "RCurl()" package.

install.package("RCurl")
library(RCurl)

#List of States to search for within the press release

state<-state.name ##R has a built in function to display state names (and state abbreviations)
state

#Website Link from Local Area Unemployment section of Bureau of Labor Statistics Website

bls.website <- getURL("http://www.bls.gov/news.release/archives/laus_11222013.htm")
class(bls.website)
bls.website


#START OF RELEVANT TEXT IN THE STRING, Table 3 contains a full list of seasonally adjusted state unemployment rates

##throw away all unecessary text before and after Table 3:

	split.1 <- strsplit(bls.website, 'Table 3.') 
	fragment.1 <- split.1[[1]][2]

	#END OF RELEVANT TEXT IN THE STRING
	fragment.2 <- strsplit(fragment.1, 'Table 4.')
	third.table <- fragment.2[[1]][1]
	
	october.2013.preliminary.unemployment.rate <- NA

#For Loop that will pull the most recent unemployment rate (the last set of numbers in each column) from the row containing that state's name
	
	for(i in 1:length(state)){
	
	term <- paste(state[i])
	state.string <- strsplit(third.table, term)
	state.string <- strsplit(state.string[[1]][2], '\n')
	state.string <- state.string[[1]][1]
	last.row <- as.data.frame(strsplit(state.string, " "))
	number <- as.numeric(as.character(last.row[dim(last.row)[1], 1]))
	october.2013.preliminary.unemployment.rate[i] <- number
...
october.2013.preliminary.unemployment.rate<-as.numeric(october.2013.preliminary.unemployment.rate)

#Now gathered all of the preliminary unemployment rates (seasonally adjusted) for October 2013

dd<-cbind.data.frame(state, october.2013.preliminary.unemployment.rate)
head(dd)
...
##histogram with variable bandwidth
library(httpuv)
library(RJSONIO)
library(shiny)
library(ggvis)
mtcars %>% 
  ggvis(~wt) %>% 
  layer_histograms(binwidth = input_slider(0, 2, step = 0.1)) ##this will open on the web
...
##Helpful R Resources Online
  
## Google's Style Guide for Writing Code in R (i.e. how to write clean, readable code that others can easily follow)

# https://google-styleguide.googlecode.com/svn/trunk/Rguide.xml

# R Instructional Guides

# http://cran.r-project.org/doc/contrib/Paradis-rdebuts_en.pdf

# http://thomasleeper.com/Rcourse/CourseOutline.html
